...
struct ClaimDetails: Decodable {
    ...
    let claimFirInformation: ClaimFirInformation
    ...
    let MessageClaimDetails: MessageClaimDetails
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case claimFirInformation = "CLAIM_FIR_INFORMATION"
        ...
        case MessageClaimDetails = "message"
    }
}
...
// MARK: - ClaimFirInformation
struct ClaimFirInformation: Decodable {
    let fir, uniqueFir, firExtension, firDate: String

    enum CodingKeys: String, CodingKey {
        case fir = "FIR"
        case uniqueFir = "UNIQUE_FIR"
        case firExtension = "FIR_EXTENSION"
        case firDate = "FIR_DATE"
    }
}
...
struct ClaimIncidentInformation: Decodable {
    let claimNo, claimUniqueNo, claimExtension, claimPartialPaymentSequence,claimDate: String
...
struct ClaimHospitalInformation: Decodable {
    ...
    let networkState,levelOfCare,DateOfAdmission,DateOfDischarge,lengthOfStay : String
...
struct ClaimProcessInformation: Decodable {
    let tpaID, reportedAmount, amount, paidAmount: String
    let rejectedCloseDate, typeOfClaim, outstandingClaimStatus, denialReasons: String
    let closeReasons,claimStatus,claimPaidDate,claimRejectedDate,claimClosedDate: String
...
// MARK: - Message
struct MessageClaimDetails: Decodable {
    let message: String
    let status: Bool
...
class JSONNullClaimDetails: Decodable, Hashable {
    ...
    public required init(from decoder: Decoder) throws {
        ...
        if !container.decodeNil() {
            throw DecodingError.typeMismatch(JSONNullClaimDetails.self, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "Wrong type for JSONNull"))
        }
    }
