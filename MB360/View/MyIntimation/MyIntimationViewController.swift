...
//        m_intimateNowTableView.separatorStyle=UITableViewCellSeparatorStyle.none
        ...
        m_intimateNowButton.layer.cornerRadius=m_intimateNowButton.frame.height/2
    ...
    func setupFontUI(){
        ...
        m_errorTitle.font = UIFont(name: FontsConstant.shared.OpenSansBold, size: CGFloat(FontsConstant.shared.contentSize25))
        m_errorTitle.textColor = FontsConstant.shared.app_FontSecondryColor
           
        m_errorDetailLbl.font = UIFont(name: FontsConstant.shared.OpenSansRegular, size: CGFloat(FontsConstant.shared.h17))
        m_errorDetailLbl.textColor = FontsConstant.shared.app_FontSecondryColor
    ...
    override func viewWillAppear(_ animated: Bool)
    {
        ...
        m_tabbarView.layer.cornerRadius=m_tabbarView.frame.size.height/2
    ...
    func getIntimationDetails()
    {
        if(isConnectedToNet())
        {
            ...
            if(employeeDetailsArray.count>0)
            {
                ...
                let urlreq = NSURL(string: WebServiceManager.getSharedInstance().getIntimatedClaimPostUrl() as String)
                ...
                print("getIntimationDetails URL: ",urlreq)
                let task = session.dataTask(with: request as URLRequest) { (data, response, error) -> Void in
                    ...
                    else
                    {
                        if let httpResponse = response as? HTTPURLResponse
                        {
                            ...
                            {
                                do {
                                    self.xmlKey = "ClmIntimationInformation"
                                    ...
                                    DispatchQueue.main.async
                                        {
                                            ...
                                            for index in 0..<self.m_intimationsArray.count
                                            {
                                                ...
                                                print("in for loop: ",intimationDict)
                                                
                                                do{
                                                self.datasource.append(ExpandedTableviewCellContent(title:intimationDict.claimant ?? "empty", otherInfo:intimationDict))
                                                }catch{
                                                    print("Error: ",error)
                                                }
                                            }
                                            ...
                                            if(self.m_intimationsArray.count==0)
                                            {
                                                ...
                                                self.m_errorImage.image=UIImage(named: "claimnotfound")
                                                self.m_errorTitle.text="No Intimations found"
        ...
        else
        {
            ...
            for index in 0..<self.m_intimationsArray.count
            {
                ...
                self.datasource.append(ExpandedTableviewCellContent(title:intimationDict.claimant!, otherInfo:intimationDict))
            }
    ...
    func getIntimationDetailsPortal()
    {
        if(isConnectedToNet())
        {
            ...
            if(employeeDetailsArray.count>0)
            {
                ...
                if let grpChildSrNo = m_employeedict?.groupChildSrNo
                {
                    groupchildsrno = String(grpChildSrNo)
                    groupchildsrno = try! AesEncryption.encrypt(groupchildsrno)
                }
                if let oeGrpBaseInfo = m_employeedict?.oe_group_base_Info_Sr_No{
                                                    ...
                                                    do
                                                    {
                                                       ...
                                                       print(json)
                                                        ...
                                                        if status{
                                                            ...
                                                            self.m_intimationsArray.reverse(); print(self.m_intimationsArray)
                                                            for index in 0..<self.m_intimationsArray.count
                                                            {
                                                                let intimationDict:Intimations = self.m_intimationsArray[index]
...
//                            self.alertForLogout()
    ...
    /*   func getIntimationDetails()
     ...
     let urlreq = NSURL(string : WebServiceManager.getSharedInstance().getIntimatedClaimUrl(empNo:employeesrno))
     ...
     print(self.m_intimationsArray.count)
     for index in 0..<self.m_intimationsArray.count
     {
     let intimationDict:Intimations = self.m_intimationsArray[index]
     ...
     if(self.m_intimationsArray.count==0)
     {
     //                                self.displayActivityAlert(title: "No Intimated Claims")
     //                                self.m_tableview.isHidden=true
     }
     }
...
//        m_intimateNowTableView.reloadData()
        ...
        for val in [vew1,vew2,vew3,vew4,vew5,vew6]{
        ...
        for dict in array
        {
            ...
            {
                m_membersArray.append(dict.personName ?? "")
            }
    ...
    func getPersonInfoForIntimationPortal(){
        if(isConnectedToNetWithAlert())
        {
            ...
            let array = DatabaseManager.sharedInstance.retrieveEmployeeDetails(productCode: "GMC")
            var groupchildsrno = String(array[0].groupChildSrNo)
            var oegrpbasinfsrno = String(array[0].oe_group_base_Info_Sr_No)
            var empSrNo = String(array[0].empSrNo)
            
            groupchildsrno = try! AesEncryption.encrypt(groupchildsrno)
            oegrpbasinfsrno = try! AesEncryption.encrypt(oegrpbasinfsrno)
            empSrNo = try! AesEncryption.encrypt(empSrNo)
        ...
        dateDropDown.selectionAction =
            {
                ...
                let array = DatabaseManager.sharedInstance.retrievePersonDetails(productCode: "GMC")
                self.m_personSrNo = array[index].personSrNo
               ...
               // self.textFields[0].text=item
                ...
                print("m_claimDetailsArray",self.m_claimDetailsArray)
               ...
               // self.m_intimateNowTableView.reloadData()
        ...
        else
        {
            if(isConnectedToNetWithAlert())
            {
                ...
                /* print(m_employeedict ?? "")
                 var urlString = WebServiceManager.getSharedInstance().getIntimateNewClaimUrl(employeesrno: (m_employeedict?.empSrNo)!, personsrno: m_personSrNo, diagnosis: m_claimDetailsArray[1], claimamount: m_claimDetailsArray[2], likelydoa: m_selectedDate, nameofhospital: m_claimDetailsArray[4], locationofhospital: m_claimDetailsArray[5])
                 
                 let customAllowedSet =  NSCharacterSet(charactersIn:",.;=\"#%/<>?@\\^`{|}").inverted
                 urlString=urlString.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
                 
                 
                 let urlreq = NSURL(string: urlString)
                 ...
                 {
                 let dict :NSDictionary = self.resultsDictArray![0] as NSDictionary
                 let intimateNo:String = dict.value(forKey: "IntimateError") as? String ?? ""
                 let alertController = UIAlertController(title: "Claim Intimated Successfully", message: "Your Intimation no is : "+intimateNo, preferredStyle: UIAlertControllerStyle.alert)
    ...
    func intimateClaim()
    {
        if(isConnectedToNetWithAlert())
        {
            ...
            if(employeeDetailsArray.count>0)
            {
                ...
                let yourXML = AEXMLDocument()
                let personNumber = String(m_personSrNo)
                var escapedClaimAmountString = m_claimDetailsArray[2].replacingOccurrences(of: ",", with: "")
                escapedClaimAmountString = m_claimDetailsArray[2].addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
                ...
                let dataRequest = yourXML.addChild(name: "DataRequest")
                dataRequest.addChild(name: "employeesrno", value:employeesrno)
                dataRequest.addChild(name: "personsrno", value:personNumber)
                dataRequest.addChild(name: "intclmdiagnosis", value:m_claimDetailsArray[1])
                dataRequest.addChild(name: "intclmclaimamount", value:escapedClaimAmountString)
                dataRequest.addChild(name: "intclmlikelydoa", value:m_selectedDate)
                dataRequest.addChild(name: "intclmnameofhospital", value:escapedhosptalNameString)
                dataRequest.addChild(name: "intclmlocationofhospital", value:escapedHospitalLocationString)
                
                print(yourXML.xml)
                let uploadData = yourXML.xml.data(using: .utf8)
                
                let urlreq = NSURL(string: WebServiceManager.getSharedInstance().getintimateClaimPostUrl() as String)
                ...
                print("intimateClaim URL: ",urlreq)
                let task = session.dataTask(with: request as URLRequest) { (data, response, error) -> Void in
                    if error != nil {
                        print("error ",error!)
                        self.hidePleaseWait()
                                    ...
                                    DispatchQueue.main.async
                                        {
                                            if((self.resultsDictArray?.count)!>0)
                                            {
                                                self.hidePleaseWait()
                                                let dict :NSDictionary = self.resultsDictArray![0] as NSDictionary
                                                let intimateNo:String = dict.value(forKey: "IntimateClaimInformation") as? String ?? ""
                                                let alertController = UIAlertController(title: "Claim intimated successfully", message: "Your claim intimation no is: "+intimateNo, preferredStyle: UIAlertControllerStyle.alert)
    ...
    func intimateClaimPortal()
    {
        if(isConnectedToNetWithAlert())
        {
            ...
            if(employeeDetailsArray.count>0)
            {
                ...
                if let gchildsrno = m_employeedict?.groupChildSrNo
                {
                ...
                let doalikelydoa = m_selectedDate
                let hospitalname = escapedhosptalNameString!
                let hospitallocation = escapedHospitalLocationString!
                ...
                let jsonDict = ["groupchildsrno":"\(groupchildsrno.URLEncoded)",
                                "oegrpbasinfsrno":"\(oegrpbasinfsrno.URLEncoded)",
                                "employeesrno":"\(employeesrno.URLEncoded)",
